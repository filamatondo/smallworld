...
$('#liste').on("keyup", function(e){
	...
	if(key == 13)
	{
		...
		if(contenu.length > 0)
		{
			...
			$('#app').append('<div data-id="'+idCard+'" class="liste liste'+idCard+'"><h2>'
            +contenu+'</h2><ul></ul><input type="text" data-card="'+idCard+'" id="carte'
            +idCard+'" placeholder="Ajouter un cmmentaire" /></div>');
			idCard++;
			$('#liste').val("");
		}
